...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-slate-100 flex">
      {/* Sidebar */}
      <aside className="w-64 border-r border-white/10 bg-black/20 backdrop-blur-xl flex flex-col p-6 fixed h-full z-10">
      ...
      <main className="flex-1 ml-64 p-8">
        {activeTab === 'dashboard' ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
              ...
              >
                <div className="flex justify-between items-center mb-6">
                  ...
                  <select className="bg-white/5 border border-white/10 rounded-lg px-3 py-1 text-sm focus:outline-none overflow-hidden">
                    <option className="bg-slate-800">All Teams</option>
                  </select>
                </div>
                <div className="h-full pb-10">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data?.teams}>
                      ...
                      <Tooltip 
                        contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #ffffff20', borderRadius: '12px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.4)' }}
                        itemStyle={{ color: '#fff' }}
                      />
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            >
              <h3 className="text-2xl font-bold mb-4">{activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Page</h3>
              <p className="text-slate-400">Currently under development for the AX Project expansion phase.</p>
              <button 
                onClick={() => setActiveTab('dashboard')}
                className="mt-8 px-6 py-2 bg-primary rounded-full font-medium hover:bg-blue-600 transition-all"
              >
                Back to Dashboard
              </button>
            </motion.div>
